...
function renderTabela() {
 ...
 html += `
 <div class="mb-3"><h5 class="secao-titulo-card mb-2">${MESES[m - 1]} <span class="text-muted" style="font-weight:400">(${doMes.length} nota${doMes.length > 1 ? 's' : ''})</span></h5><div class="card"><div class="table-responsive"><table class="table table-sm table-hover mb-0" style="font-size:.82rem"><thead><tr>${COLS.map((c) => `<th class="${c.tipo === 'num' ? 'text-end' : ''}">${c.t}</th>`).join('')}<th class="no-print text-center">Ações</th></tr></thead><tbody>
 ${doMes.map((d) => `<tr>
 ${COLS.map((c) => {
 let v = d[c.k]
 ...
 <td class="no-print text-center" style="white-space:nowrap"><button class="btn btn-sm btn-outline-primary py-0 px-1" onclick="editarNota(${d.id})" title="Editar">✏️</button><button class="btn btn-sm btn-outline-danger py-0 px-1" onclick="excluirNota(${d.id})" title="Excluir">🗑</button></td></tr>`).join('')}
 <tr class="fw-bold" style="background:#fbeaea"><td colspan="8" class="text-end">Total ${MESES[m - 1]}:</td><td class="text-end">${money(totValor)}</td><td class="text-end">${peso(totPeso)}</td><td colspan="3"></td><td class="no-print"></td></tr></tbody></table></div></div></div>`
 }
 cont.innerHTML = html
}
...
function renderResumo() {
 ...
 <div class="row g-3 mb-3"><div class="col-6 col-md-3"><div class="card"><div class="card-body text-center py-3"><div class="small text-muted">Valor Comercializado</div><div class="fw-bold fs-5 text-success">R$ ${money(totalValor)}</div></div></div></div><div class="col-6 col-md-3"><div class="card"><div class="card-body text-center py-3"><div class="small text-muted">Kilos Produzidos</div><div class="fw-bold fs-5">${peso(totalPeso)} kg</div></div></div></div><div class="col-6 col-md-3"><div class="card"><div class="card-body text-center py-3"><div class="small text-muted">Notas no ano</div><div class="fw-bold fs-5">${totalNotas}</div></div></div></div><div class="col-6 col-md-3"><div class="card"><div class="card-body text-center py-3"><div class="small text-muted">Média por nota</div><div class="fw-bold fs-5">R$ ${money(totalNotas ? totalValor / totalNotas : 0)}</div></div></div></div></div><div class="card mb-4"><div class="card-body"><h5 class="secao-titulo-card mb-2">Vendas por mês — ${anoAtual}</h5><div class="table-responsive"><table class="table table-sm mb-0" style="font-size:.85rem"><thead><tr><th>Mês</th><th class="text-end">Notas</th><th class="text-end">Valor comercializado</th><th class="text-end">Peso (kg)</th></tr></thead><tbody>${porMes.filter((m) => m.qtd).map((m) => `<tr><td>${m.nome}</td><td class="text-end">${m.qtd}</td><td class="text-end">R$ ${money(m.valor)}</td><td class="text-end">${peso(m.peso)}</td></tr>`).join('')}
 <tr class="fw-bold" style="background:#fbeaea"><td>TOTAL</td><td class="text-end">${totalNotas}</td><td class="text-end">R$ ${money(totalValor)}</td><td class="text-end">${peso(totalPeso)}</td></tr></tbody></table></div></div></div>`
}
...
function exportarPDF() {
 ...
 const tp = doMes.reduce((s, d) => s + (Number(d.peso) || 0), 0)
 html += `
 <div style="background:${AZUL};color:#fff;font-weight:bold;padding:5px 8px;margin-top:10px;font-size:12px">${MESES[m - 1].toUpperCase()}</div><table style="width:100%;border-collapse:collapse;margin-bottom:2px"><thead><tr>${COLS.map((c) => `<th style="${thBase}">${c.t}</th>`).join('')}</tr></thead><tbody>
 ...
 <tr><td colspan="8" style="background:${TOTAL};border:${BORDA};padding:4px 6px;font-size:10px;font-weight:bold;text-align:right">TOTAL ${MESES[m - 1]}</td><td style="background:${TOTAL};border:${BORDA};padding:4px 6px;font-size:10px;font-weight:bold;text-align:right">${money(tv)}</td><td style="background:${TOTAL};border:${BORDA};padding:4px 6px;font-size:10px;font-weight:bold;text-align:right">${money(tp)}</td><td colspan="3" style="background:${TOTAL};border:${BORDA}"></td></tr></tbody></table>`
 }

 const totalValor = dados.reduce((s, d) => s + (Number(d.valor_nfe) || 0), 0)
 const totalPeso = dados.reduce((s, d) => s + (Number(d.peso) || 0), 0)
 html += `
 <div style="background:${AZUL};color:#fff;font-weight:bold;padding:5px 8px;margin-top:14px;font-size:12px">RELATÓRIO ANUAL</div><table style="border-collapse:collapse;margin-bottom:10px"><tr><td style="${tdBase('left')}">Valor Comercializado</td><td style="${tdBase('right')}">R$ ${money(totalValor)}</td></tr><tr><td style="${tdBase('left')}">Kilos Produzidos</td><td style="${tdBase('right')}">${money(totalPeso)} kg</td></tr><tr><td style="${tdBase('left')}">Total de Notas</td><td style="${tdBase('right')}">${dados.length}</td></tr></table>`
...
function montarInterface() {
 ...
 cont.innerHTML = `
 <div class="card mb-4 no-print"><div class="card-body"><div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3"><h5 class="secao-titulo-card mb-0">➕ Lançar nota fiscal</h5><div class="d-flex align-items-center gap-2"><label class="small fw-semibold mb-0">Ano:</label><select id="sel-ano" class="form-select form-select-sm" style="width:100px;padding-right:30px"></select><button id="btn-add-ano" type="button" class="btn btn-sm btn-outline-secondary" title="Adicionar um ano novo">+ Ano</button></div></div><div class="row g-2"><div class="col-6 col-md-2"><label class="form-label small mb-0">Data</label><input type="date" id="f-data" class="form-control form-control-sm"></div><div class="col-6 col-md-2"><label class="form-label small mb-0">Mês (se sem data)</label><select id="f-mes" class="form-select form-select-sm"><option value="">—</option>${MESES.map((m, i) => `<option value="${i + 1}">${m}</option>`).join('')}</select></div><div class="col-6 col-md-2"><label class="form-label small mb-0">NF</label><input type="text" id="f-nf" class="form-control form-control-sm"></div><div class="col-6 col-md-2"><label class="form-label small mb-0">Fatura</label><input type="text" id="f-fatura" class="form-control form-control-sm"></div><div class="col-6 col-md-2"><label class="form-label small mb-0">Núm. DUE</label><input type="text" id="f-num_due" class="form-control form-control-sm"></div><div class="col-6 col-md-2"><label class="form-label small mb-0">Data DUE</label><input type="date" id="f-data_due" class="form-control form-control-sm"></div><div class="col-6 col-md-2"><label class="form-label small mb-0">Nº Conhec. Emb.</label><input type="text" id="f-num_conhecimento" class="form-control form-control-sm"></div><div class="col-6 col-md-2"><label class="form-label small mb-0">Data Conhec.</label><input type="date" id="f-data_conhecimento" class="form-control form-control-sm"></div><div class="col-6 col-md-2"><label class="form-label small mb-0">Tipo</label><select id="f-tipo" class="form-select form-select-sm"><option value="">—</option><option>BL</option><option>CRT</option><option>DDE</option><option>SD</option></select></div><div class="col-6 col-md-2"><label class="form-label small mb-0">Valor da NFE</label><input type="number" step="any" id="f-valor_nfe" class="form-control form-control-sm"></div><div class="col-6 col-md-2"><label class="form-label small mb-0">Peso (kg)</label><input type="number" step="any" id="f-peso" class="form-control form-control-sm"></div><div class="col-6 col-md-2"><label class="form-label small mb-0">Vendedor</label><input type="text" id="f-vendedor" class="form-control form-control-sm"></div><div class="col-6 col-md-3"><label class="form-label small mb-0">Produto</label><input type="text" id="f-produto" class="form-control form-control-sm"></div><div class="col-6 col-md-2"><label class="form-label small mb-0">País</label><input type="text" id="f-pais" class="form-control form-control-sm"></div></div><div class="mt-3 d-flex gap-2"><button id="btn-salvar-nota" class="btn btn-ok-grande">➕ Lançar nota</button><button id="cancelar-edicao" class="btn btn-outline-secondary" style="display:none">Cancelar</button></div></div></div><div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3"><h4 class="fw-bold mb-0">Faturamento ${anoAtual}</h4><div class="d-flex gap-2 no-print"><button id="btn-excel" class="btn btn-sm btn-outline-success">Exportar Excel</button><button id="btn-pdf" class="btn btn-sm btn-outline-danger">Exportar PDF</button></div></div><div id="resumo-contabil"></div><div id="tabela-contabil"></div>
 `
 ...
 const resp = prompt('Digite o ano que deseja abrir (ex.: 2027):', String(atual + 1))
 if (resp === null) return
