...
const NOMES = {
  ...
  'auxiliarexp@pietrobon.com.br': { nome: 'Wesley',        iniciais: 'WS' }
}
...
function renderGruposStatus(items) {
  const grupos = COLUNAS
    .map(col => ({ col, items: items.filter(t => t.coluna === col) }))
    .filter(g => g.items.length > 0)

  if (!grupos.length) {
    return `<div class="empty-coluna">
  ...
  return grupos.map(g => `
    <div class="status-grupo">
      <div class="status-divider" style="color:${COR_COL[g.col]}">
        <span class="status-dot" style="background:${COR_COL[g.col]}"></span>
        ${LABEL_COL[g.col]}
...
function renderColunaUsuario(email, filtradas) {
  ...
  const labelNome = isVoce ? `${u.nome} <span class="grupo-voce">você</span>` : u.nome
  ...
  return `<div class="col-usuario" data-email="${escHtml(email)}">
    <div class="col-usuario-header${isVoce ? ' col-header-voce' : ''}">
      ...
      <span class="col-usuario-badge">${minhas.length}</span>
    </div>
    <div class="col-usuario-body">
      ${renderGruposStatus(minhas)}
      <button class="btn-nova-tarefa" onclick="abrirModal(null,'a_fazer','${escHtml(email)}')">＋ Adicionar tarefa</button>
    </div>
  </div>`
}

function renderColunaSemResponsavel(items) {
  return `<div class="col-usuario col-sem-responsavel" data-email="">
    <div class="col-usuario-header">
      <span class="col-usuario-avatar" style="background:#94a3b8;font-size:.9rem">?</span>
      <span class="col-usuario-nome" style="color:#64748b">Sem responsável</span>
      <span class="col-usuario-badge">${items.length}</span>
    </div>
    <div class="col-usuario-body">
      ${renderGruposStatus(items)}
      <button class="btn-nova-tarefa" onclick="abrirModal(null,'a_fazer','')">＋ Adicionar tarefa</button>
    </div>
  </div>`
}
...
window.renderizarBoard = function() {
  ...
  let html = usuariosParaMostrar.map(email => renderColunaUsuario(email, filtradas)).join('')

  if (!filtroResp) {
    const semDono = filtradas.filter(t => !t.responsavel)
    if (semDono.length) html += renderColunaSemResponsavel(semDono)
  }
...
function setupColunasDrop() {
  document.querySelectorAll('.col-usuario').forEach(col => {
    ...
    col.ondrop = async e => {
      ...
      if (!t || t.responsavel === (emailCol || null)) return
      t.responsavel = emailCol || null
      renderizarBoard()
...
window.salvarTarefa = async function() {
  ...
  if (!responsavel) { alert('Selecione um responsável para a tarefa.'); document.getElementById('modal-responsavel').focus(); return }
