...
export function resumoDeclaracaoHtml(pedidos) {
 ...
 return `
 <div class="resumo-alerta-declaracao-topo"><div style="font-size:1.1rem;margin-bottom:6px">⏰ ${emAlerta.length} PI(s) COM ESTOQUE NÃO DECLARADO (48h+)</div><div style="font-weight:600;opacity:.95;margin-bottom:8px">PIs com produtos cadastrados há mais de 48h e ainda sem informe salvo no almoxarifado:</div><div>${chips}</div></div>`
}
...
export function resumoAlertasHtml(pedidos) {
 ...
 return `
 <div class="resumo-alerta-topo"><div style="font-size:1.1rem;margin-bottom:6px">${emAlerta.length} PI(s) EM ALERTA MÁXIMO DE EMBARQUE</div><div style="font-weight:600;opacity:.95;margin-bottom:8px">PIs próximas da data de embarque (ou vencidas) que ainda não estão prontas para produzir:</div><div>${chips}</div></div>`
}