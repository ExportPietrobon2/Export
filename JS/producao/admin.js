...
import { piEmAlerta, bannerAlertaHtml, resumoAlertasHtml, piNaoDeclarada, bannerDeclaracaoHtml, resumoDeclaracaoHtml, seloPrazoDeclaracaoPiHtml, seloPrazoDeclaracaoHtml } from '/JS/core/alertas.js'
...
function renderAlmoxarifado(produtos) {
 ...
 return `
 <div class="card border-0 bg-light rounded-3 p-3 mb-2"><div class="d-flex align-items-center gap-2 flex-wrap mb-2"><strong>${produto.produto}</strong><span class="badge bg-secondary">${formatarQuantidade(produto.quantidade)}</span><span class="badge ${liberado ? 'bg-success' : 'bg-danger'}">${status}</span>
...
function renderEntradasB2Geral(entradas) {
 ...
 return `
 <div class="border rounded-3 p-3 mb-2 bg-light"><div class="d-flex justify-content-between flex-wrap gap-1 mb-1"><span class="${e.produto ? 'fw-semibold small' : 'text-muted small fst-italic'}">${e.produto || 'Produto não informado'}</span><span class="small text-muted">${data}</span></div>
...
function renderCard(pedido) {
 ...
 </div><div class="mt-1 d-flex align-items-center gap-2 flex-wrap"><span class="badge ${liberado ? 'bg-success' : bloqueado ? 'bg-danger' : 'bg-secondary'}">${status}</span>
 ...
 </div></div><div class="d-flex gap-2 flex-wrap"><button class="btn btn-sm btn-outline-danger btn-expandir" data-id="${pedido.id}">${aberto ? 'Fechar ▴' : 'Ver detalhes ▾'}</button>
 ...
 <div class="secao-titulo-card mb-2">Cobrar data de embarque (para o gerente)</div><div class="input-group input-group-sm"><input type="text" class="form-control coment-embarque" data-id="${pedido.id}" placeholder="Ex.: Favor declarar a data de embarque desta PI" value="${(pedido.comentario_embarque || '').replace(/"/g, '&quot;')}"><button class="btn btn-outline-primary" onclick="salvarComentarioEmbarque(${pedido.id})">Enviar</button></div>
 `
...
async function carregar() {
 ...
 <div class="card-body"><div class="secao-titulo-card mb-3">Todas as Entradas do B2</div>
