...
function montarCalculadora(wrapper) {
  ...
  card.innerHTML = `
    <div class="card-body">
      <h5 class="secao-titulo-card mb-3">🧮 Calculadora de rendimento</h5>
      <div class="row g-3">
        <div class="col-12 col-lg-5">
          <label class="form-label fw-semibold small">Produto</label>
          <select id="calc-produto" class="form-select">${optgroups}</select>
        </div>
        <div class="col-6 col-lg-3">
          <label class="form-label fw-semibold small">Material disponível (kg)</label>
          <input type="number" id="calc-kg" class="form-control" placeholder="0" min="0" step="any">
        </div>
        <div class="col-6 col-lg-4">
          <label class="form-label fw-semibold small" id="calc-alvo-label">Produção desejada (pacotes)</label>
          <input type="number" id="calc-alvo" class="form-control" placeholder="0" min="0" step="any">
        </div>
      </div>
      <div class="form-check mt-3">
        <input class="form-check-input" type="checkbox" id="calc-perda">
        <label class="form-check-label small" for="calc-perda">Descontar 3% de perda</label>
      </div>
      <div id="calc-resultado" class="mt-3"></div>
    </div>`
...
function renderizarTabela(filtro) {
  ...
  DADOS.forEach((secao) => {
    ...
    const cabecalho = eRotulo
      ? `<th class="text-center" style="white-space:nowrap">Peso ref.</th>
         <th class="text-center" style="white-space:nowrap">kg de produto / kg de rótulo</th>
         <th class="text-center" style="white-space:nowrap">kg de rótulo / 100 kg produto</th>`
      : `<th class="text-center" style="white-space:nowrap">Peso da embalagem</th>
         <th class="text-center" style="white-space:nowrap">Pacotes por kg</th>
         <th class="text-center" style="white-space:nowrap">kg p/ 1.000 pacotes</th>`
    ...
    const linhas = itens.map((i) => {
      if (eRotulo) {
        ...
        return `<tr>
          <td>${i.produto}</td>
          <td class="text-center">${fmt(i.pesoG, 2)} g</td>
          <td class="text-center fw-semibold text-success">${fmt(i.base, 2)} kg</td>
          <td class="text-center">${fmt(kgRotPor100, 2)} kg</td>
        </tr>`
      }
      ...
      return `<tr>
        <td>${i.produto}</td>
        <td class="text-center">${fmt(i.pesoG, 2)} g</td>
        <td class="text-center fw-semibold text-success">${fmtInt(i.base)} pct</td>
        <td class="text-center">${fmt(i.pesoG, 2)} kg</td>
      </tr>`
    }).join('')
      ...
      <div class="card">
        <div class="table-responsive">
          <table class="table table-sm table-hover mb-0">
            <thead><tr><th>Produto</th>${cabecalho}</tr></thead>
            <tbody>${linhas}</tbody>
          </table>
        </div>
      </div>`
