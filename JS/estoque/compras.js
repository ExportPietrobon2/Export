...
async function carregarCompras() {
 ...
 </div></div><span class="badge ${statusCor[c.status] || 'bg-secondary'}">${statusLabel[c.status] || c.status}</span></div><div class="small text-muted mb-2">
 Compra: ${dataBR(c.data_compra)} · Chegada prevista: <span class="${atraso ? 'text-danger fw-bold' : 'fw-semibold'}">${dataBR(c.data_prevista)}</span></div>
 ${podeComprar ? `<div class="d-flex gap-2 flex-wrap align-items-center"><select class="form-select form-select-sm" style="max-width:160px" onchange="mudarStatusCompra(${c.id}, this.value)">${opcoesStatus}</select>
 ${c.status !== 'recebido' ? `<button class="btn btn-sm btn-pietrobon" onclick="receberCompra(${c.id})">Recebida</button>` : ''}
 <button class="btn btn-sm btn-outline-danger" onclick="excluirCompra(${c.id})">Excluir</button></div><div class="input-group input-group-sm mt-2"><input type="text" class="form-control obs-compra" data-id="${c.id}" placeholder="Observação (ex.: Verificar estoque de caixas)" value="${(c.observacoes || '').replace(/"/g, '&quot;')}"><button class="btn btn-outline-secondary" onclick="salvarObsCompra(${c.id})">Enviar observação</button></div>` : (c.observacoes ? `<div class="small text-muted mt-1">${c.observacoes}</div>` : '')}
 </div></div>`
 }).join('')
}
...
async function carregarSugestoes() {
 ...
 <div class="card border-0 shadow-sm mb-2"><div class="card-body"><div class="fw-bold mb-1">PI ${numeroPi}${info.cliente ? ` — ${info.cliente}` : ''}
 ${info.data_embarque ? `<span class="small text-muted">· ${dataBR(info.data_embarque)}</span>` : ''}
 </div>
 ${info.itens.map((i) => `
 <div class="small d-flex justify-content-between border-bottom py-1"><span>${i.produto} <span class="badge bg-light text-dark border">${tipoLabel[i.insumo_tipo] || i.insumo_tipo}</span></span><span class="text-danger fw-semibold">${i.insumo_tipo === 'etiqueta' ? `${i.sobra} un (baixo)` : 'em falta'}</span></div>`).join('')}
 </div></div>`).join('')}`
}
...
async function carregarEntradasB2() {
 ...
 return `
 <div class="card border-0 shadow-sm mb-2"><div class="card-body"><div class="d-flex justify-content-between flex-wrap gap-1 mb-1"><span class="${e.produto ? 'fw-semibold' : 'text-muted fst-italic'}">${e.produto || 'Produto não informado'}</span><span class="small text-muted">${data}</span></div>
