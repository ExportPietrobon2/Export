...
import { iniciarReferencia } from '/JS/estoque/referencia.js'
import { seloPrazoDeclaracaoHtml } from '/JS/core/alertas.js'
import { iniciarPedidosCompra } from '/JS/core/demandas.js'
...
function cardInsumoHtml(chave, insumo, produto) {
 ...
 return `<div class="col-6 col-md-3"><div class="card-insumo-almox h-100"><h4>${NOME_INSUMO[chave]}</h4><label class="form-label small fw-semibold">Sobra (${unidade})</label><input type="number" class="form-control" data-produto="${produto.id}" data-campo="sobra" data-tipo="${chave}" value="${sobra}" placeholder="0">
 ${chave === 'embalagem' ? `<label class="form-label small fw-semibold mt-2">Pacotes possíveis</label><input type="number" class="form-control" data-produto="${produto.id}" data-campo="quantidade_por_pacote" data-tipo="${chave}" value="${insumo.quantidade_por_pacote || 0}" placeholder="0">` : ''}
 ${resultado}
 </div></div>`
}
...
function rotuloRowHtml(produtoId, r) {
 ...
 return `<div class="rotulo-row card-insumo-almox p-2 mb-2" data-produto="${produtoId}"><div class="d-flex justify-content-between align-items-center mb-1"><span class="small fw-semibold">Rótulo</span><button type="button" class="btn btn-sm btn-outline-danger py-0 px-2 btn-remover-rotulo" style="font-size:.75rem">✕</button></div><input type="text" class="form-control form-control-sm mb-1 rotulo-nome" list="lista-rotulos" placeholder="Nome do rótulo (ex.: Pirulito)" value="${nome}"><div class="row g-1"><div class="col-6"><label class="form-label small mb-0">Sobra (kg)</label><input type="number" class="form-control form-control-sm rotulo-sobra" value="${sobra}" placeholder="0" min="0" step="any"></div><div class="col-6"><label class="form-label small mb-0">Pacotes</label><input type="number" class="form-control form-control-sm rotulo-pac" value="${pac}" placeholder="0" min="0" step="any"></div></div></div>`
}
...
function salvarRascunho(produtoId) {
 try { localStorage.setItem(chaveRascunho(produtoId), JSON.stringify(coletarFormulario(produtoId))) } catch (e) {  }
}

function limparRascunho(produtoId) {
 try { localStorage.removeItem(chaveRascunho(produtoId)) } catch (e) {  }
}
...
async function carregarRecebimentos() {
 ...
 return `
 <div class="border rounded-3 p-3 mb-2 bg-light" id="rec-entrada-${e.id}"><div class="d-flex justify-content-between align-items-start mb-1 flex-wrap gap-1"><span class="small text-muted">${data}</span>
 ${!window._convidado ? `<button class="btn btn-sm btn-outline-primary py-0 px-2" style="font-size:0.75rem;border-radius:8px" onclick="editarProdutoEntrada(${e.id}, '${(e.produto || '').replace(/'/g, "\'")}')">${e.produto ? 'Editar produto' : 'Adicionar produto'}</button>` : ''}
 </div><div id="rec-produto-label-${e.id}" class="${e.produto ? 'fw-semibold small mb-1' : 'text-muted small fst-italic mb-1'}">${e.produto || 'Produto não informado'}</div><div id="rec-produto-form-${e.id}" style="display:none" class="mb-2"><div class="d-flex gap-2"><input type="text" id="rec-produto-input-${e.id}" class="form-control form-control-sm" value="${e.produto || ''}" placeholder="Nome do produto"><button class="btn btn-sm btn-pietrobon px-3" onclick="salvarProdutoEntrada(${e.id})">✔</button><button class="btn btn-sm btn-outline-secondary px-3" onclick="cancelarProdutoEntrada(${e.id})">✕</button></div></div><div class="d-flex justify-content-between align-items-center gap-2 mb-1 flex-wrap"><div id="rec-local-label-${e.id}" class="${e.localizacao ? 'small' : 'text-muted small fst-italic'}">${e.localizacao || 'Localização não informada'}</div>
 ...
 </div><div id="rec-local-form-${e.id}" style="display:none" class="mb-2"><div class="d-flex gap-2"><input type="text" id="rec-local-input-${e.id}" class="form-control form-control-sm" value="${e.localizacao || ''}" placeholder="Ex: Galpão 2, prateleira A3"><button class="btn btn-sm btn-pietrobon px-3" onclick="salvarLocalizacaoEntrada(${e.id})">✔</button><button class="btn btn-sm btn-outline-secondary px-3" onclick="cancelarLocalizacaoEntrada(${e.id})">✕</button></div></div><div class="d-flex gap-2 flex-wrap">
 ...
 if (btnEditar) btnEditar.textContent = valor ? 'Editar produto' : 'Adicionar produto'
 }
...
async function carregarEstoqueGeral() {
 ...
 <div class="mt-2 pt-2 border-top"><div class="small fw-semibold text-muted mb-1">Vínculos:</div>
 ${e._vinculos.map((v) => `
 <div class="d-flex justify-content-between align-items-center flex-wrap gap-1 mb-1" id="vinculo-${v.id}"><div class="d-flex gap-1 flex-wrap align-items-center"><span class="badge bg-danger">PI ${v.numero_pi}</span>
 ...
 ${!window._convidado ? `
 <div class="d-flex gap-1"><button class="btn btn-sm btn-outline-primary py-0 px-2" style="font-size:0.75rem;border-radius:8px" onclick="toggleEditarVinculo(${v.id}, ${e.id})">✏️</button><button class="btn btn-sm btn-outline-danger py-0 px-2" style="font-size:0.75rem;border-radius:8px" onclick="deletarVinculo(${v.id})">🗑</button></div>` : ''}
 </div><div class="edit-vinculo-${v.id} mb-2" style="display:none"><div class="row g-2 mb-2"><div class="col-12"><select class="form-select form-select-sm edit-pi-${v.id}">
 ...
 card.innerHTML = `
 <div class="card-body pb-2"><div class="d-flex justify-content-between align-items-start mb-2 flex-wrap gap-1"><div>
 ...
 <div class="small text-muted">${data}</div></div><span class="badge ${totDisp === 0 ? 'bg-success' : totalVinculado > 0 ? 'bg-warning text-dark' : 'bg-secondary'}">
 ${totDisp === 0 ? 'Totalmente vinculado' : totalVinculado > 0 ? 'Parcialmente vinculado' : '○ Não vinculado'}
 </span></div><div class="row g-2 mb-2">
 ${e.embalagem_kg > 0 ? `
 <div class="col-auto"><div class="card-insumo-almox px-3 py-2 text-center"><div class="small text-muted">Embalagem</div><div class="fw-bold">${e.embalagem_kg} kg</div>
 ...
 ${e.rotulo_kg > 0 ? `
 <div class="col-auto"><div class="card-insumo-almox px-3 py-2 text-center"><div class="small text-muted">Rótulo</div><div class="fw-bold">${e.rotulo_kg} kg</div>
 ...
 ${e.pallet_caixas > 0 ? `
 <div class="col-auto"><div class="card-insumo-almox px-3 py-2 text-center"><div class="small text-muted">Pallets</div><div class="fw-bold">${e.pallet_caixas}</div>
 ...
 Vincular a uma PI
 </button><div id="form-vincular-${e.id}" style="display:none" class="mt-2 border rounded-3 p-3 bg-light"><div class="mb-2"><label class="form-label small fw-semibold mb-1">PI</label><select id="vinc-pi-${e.id}" class="form-select form-select-sm"><option value="">Selecione</option>
 ${(pis || []).map((p) => `<option value="${p.id}">PI ${p.numero_pi}${p.cliente ? ' — ' + p.cliente : ''}</option>`).join('')}
 </select></div><div class="row g-2 mb-2">
